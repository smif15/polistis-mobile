...
@Composable
fun RiwayatDokterScreen(
    token: String = "",
    riwayatDokterViewModel: RiwayatDokterViewModel = viewModel(factory = RiwayatDokterViewModel.Factory)
) {
    val riwayats = dummyPengajuan()

    val scope = rememberCoroutineScope()
    val laporans by riwayatDokterViewModel.laporansList.collectAsState()
    riwayatDokterViewModel.setToken(token)
    riwayatDokterViewModel.getDaftarPengajuanDokter()
    ...
    Column(
        ...
        Column(modifier = Modifier
            ...
            Text(text = "Riwayat Pemeriksaan",
                style= MaterialTheme.typography.titleLarge, color = White,
                modifier = Modifier.padding(25.dp)
            )
        }
        LaporanDokterItem(laporans = laporans)

    }
}

@Composable
fun LaporanDokterItem(
...
@Composable
fun RiwayatDokterScreenPreview() {
    PolistisTheme {
        RiwayatDokterScreen()
    }
}